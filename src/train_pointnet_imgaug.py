...
from utils.loss import batch_NN_loss, jaccard_loss
from utils.utilss import soft_to_hard_pred
...
# get the generators for training, validation and testing process(A: source, B: target)
def get_generators(ids_train, ids_valid, ids_train_lge, ids_valid_lge, batch_size=16, n_samples=1000, crop_size=224,
    ...
    trainA_generator = DataGenerator_PointNet(df=ids_train, channel="channel_first", apply_noise=True, phase="train",
                                              apply_online_aug=args.offaug, aug2=args.aug2,
                                              batch_size=batch_size, source="source", crop_size=crop_size,
                                              n_samples=n_samples, match_hist=mh, rev_data=args.rev_data, offclahe=args.offclahe)
    validA_generator = DataGenerator_PointNet(df=ids_valid, channel="channel_first", apply_noise=False, phase="valid",
                                              ...
                                              batch_size=batch_size, source="source", crop_size=crop_size,
                                              n_samples=-1, match_hist=mh, rev_data=args.rev_data, offclahe=args.offclahe)
...
# valid model in one data set (training, validation or testing / source or target)
def valid_model_with_one_dataset(seg_model, data_generator, hd=False):
    ...
    # collect the mean values of the metrics
    output = {}
...
# valid model with source validation set, target validation set and target testing set
...
@timeit
def train_epoch(model_gen, model_dis2, model_dis3, model_dis1=None,
                optim_gen=None, optim_dis2=None, optim_dis3=None, optim_dis1=None,
                trainA_iterator=None, trainB_iterator=None):
    """

    :param model_gen: the model of DRUnet
    :param model_dis2: the entropy discriminator
    :param model_dis3: the point-cloud discriminator
    :param model_dis1: the outer space discriminator
    :param optim_gen: the optimizer of the Unet
    :param optim_dis2: the optimizer of the entropy discriminator
    :param optim_dis3: the optimizer of the point-cloud discriminator
    :param optim_dis1: the optimizer of the outer space discriminator
    :param trainA_iterator: the training generator for source data set
    :param trainB_iterator: the training generator for target data set
    :return: losses and metrics
    """
    source_domain_label = 1
    target_domain_label = 0
    smooth = 1e-7
    # turn all the models to "train mode"
    model_gen.train()
    if args.d1:
        model_dis1.train()
    if args.d2:
    ...
    d1_acc1, d1_acc2, d2_acc1, d2_acc2, d3_acc1, d3_acc2 = [], [], [], [], [], []

    for (imgA, maskA, vertexA), (imgB, _, vertexB) in zip(trainA_iterator, trainB_iterator):
        # 1. train generator with a batch of random images
        if args.d1:
        ...
        if args.d3:
            ...
            D_out3 = model_dis3(vertT.transpose(2, 1))[0]
            loss_adv_diff_point = args.dr * F.binary_cross_entropy_with_logits(D_out3, torch.FloatTensor(D_out3.data.size()).fill_(
                source_domain_label).cuda())
        ...
        if args.d3:
            vertS = vertS.detach()
            D_out3 = model_dis3(vertS.transpose(2, 1))[0]
        ...
        if args.d3:
            vertT = vertT.detach()
            D_out3 = model_dis3(vertT.transpose(2, 1))[0]
            loss_D_diff_3 = F.binary_cross_entropy_with_logits(D_out3, torch.FloatTensor(D_out3.data.size()).fill_(
                target_domain_label).cuda())
            # loss_D_diff = loss_D_diff_1 + loss_D_diff_2
            loss_D_diff_3.backward()
            D_out3 = torch.sigmoid(D_out3.detach()).cpu().numpy()
            D_out3 = np.where(D_out3 >= .5, 1, 0)
...
def print_epoch_result(train_result, valid_result, epoch, max_epochs):
    ...
    if args.d3:
        dis3_acc1, dis3_acc2 = train_result["dis3_acc1"], train_result['dis3_acc2']
        print_msg_line2 += f'disctor3_train_acc1: {dis3_acc1: 5f} ' + f'disctor3_train_acc2: {dis3_acc2: 5f} '
...
def main(batch_size=24, n_samples=1000, n_epochs=200):
    if args.rev_data:
        ids_train = ImageProcessor.split_data('./../input_aug/rev_data/aug_trainB.csv')
        ids_valid = ImageProcessor.split_data('./../input_aug/rev_data/testB.csv')
        ids_train_lge = ImageProcessor.split_data("./../input_aug/rev_data/aug_trainA.csv")
        ids_valid_lge = ImageProcessor.split_data("./../input_aug/rev_data/testA.csv")
    else:
    ...
    # 3. model
    model_gen = Segmentation_model_Point(filters=32, pointnet=args.d3, drop=args.drop)
    ...
    model_dis1 = None
    if args.d1:
        model_dis1 = UncertaintyDiscriminator(in_channel=4, softmax=args.softmax, sigmoid=args.sigmoid).cuda()
    model_dis2 = None
    if args.d2:
        model_dis2 = UncertaintyDiscriminator(in_channel=4).cuda()
    model_dis3 = None
    if args.d3:
        model_dis3 = PointNetCls(k=2).cuda()
    ...
    if args.d2:
        ...
        modelcheckpoint_dis2 = ModelCheckPointCallback(n_epochs=n_epochs,
                                                       ...
                                                       entire_model=False)
    if args.d3:
        d3_weight_dir = root_directory + 'point_dis_{}.pt'.format(appendix)
        best_d3_weight_dir = root_directory + 'best_point_dis_{}.pt'.format(appendix)
    ...
    for epoch in tqdm.trange(n_epochs, desc='Train', ncols=80):
        train_result = train_epoch(model_gen=model_gen, model_dis2=model_dis2,
                                   model_dis3=model_dis3, model_dis1=model_dis1,
                                   optim_gen=optim_gen, optim_dis2=optim_dis2,
                                   optim_dis3=optim_dis3, optim_dis1=optim_dis1,
        ...
        if args.d3:
            modelcheckpoint_dis3.step(monitor=monitor_score, model=model_dis3, epoch=epoch + 1)
        if (datetime.now() - start_time).seconds > max_duration:
            break
        if args.offdecay:
    ...
    if to_write_summary:
        ...
        if args.d3:
            i = 1
            for p_s_loss, p_t_loss, a1, a2 in zip(pcloud_s_loss, pcloud_t_loss, d3_acc1, d3_acc2):
...
if __name__ == '__main__':
    ...
    max_duration = 24 * 3600 - 10 * 60  # 85800 seconds
    seed = 0
    ...
    parser.add_argument("-offclahe", help='whether not to use clahe', action='store_false')
    ...
    parser.add_argument("-d3", help='whether to use pointnet', action='store_true')
    ...
    parser.add_argument("-d3lr", help="the learning rate for pointnet discriminator", type=float, default=2.5e-5)
    ...
    parser.add_argument("-sigmoid",
                        help="whether to apply sigmoid as the first layer of the out space discriminator and feature discriminator",
                        action='store_true')
    parser.add_argument("-dr", help="the ratio of the discriminators loss for the unet", type=float, default=.01)
    parser.add_argument("-rev_data", help="whether to reverse source and target data", action='store_true')
    ...
    assert args.bs != 1, 'batch size should not be one.'
